...
public class Parser {
    ...
    private void statement_list() {
        ...
        is(TK.PRINT) || is(TK.DO) || is(TK.IF) || is(TK.MONEY) ) {
            statement();
        }
    }
    ...
    // PART 5: our for loop funciton
    private void forLoop(){
        scan();
        if( tok.kind != TK.NUM ) {
    	    System.err.println( "mustbe: want " + TK.NUM + ", got " + tok);
    	    parse_error( "missing token (mustbe)" );
    	}
    	System.out.println( "int y_i;" );
    	System.out.println( "for( y_i = 0; y_i < " + tok.string + "; y_i++){" );
    	scan();
        mustbe(TK.THEN);
        block();
        mustbe(TK.QUESTION);
